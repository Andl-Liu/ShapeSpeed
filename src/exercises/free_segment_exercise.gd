...
func _compute_rating(error_px: float) -> BaseExercise.Rating:
	var tiers: Dictionary = Settings.get_rating_tiers("free_segment")
...
func validate() -> Dictionary:
	...
	# 评分容差与"过关"档对齐
	var pass_px: float = Settings.get_pass_threshold("free_segment")
	var accuracy: float = clampf(1.0 - avg_error / pass_px, 0.0, 1.0)
